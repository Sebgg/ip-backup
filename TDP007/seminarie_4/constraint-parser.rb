...
class ConstraintParser < Parser
    ...
    @parser=super("constraints") do
      ...
      rule :expr do
        ...
        match(:expr, '-', :term) do |a, _, b|
          conn_a,conn_b,conn_c=get_connectors(a,'-',b)
          # a-b=c <=> a=b+c
          Adder.new(conn_a,conn_b,conn_c).swap_op #ny
        end
      ...
      rule :term do
        ...
        match(:term, '/', :atom) do |a, _, b|
          conn_a,conn_b,conn_c=get_connectors(a,'/',b)
          # a/b=c <=> b*c=a
          Multiplier.new(conn_a,conn_b,conn_c).swap_op #ny
        end
  ...
  def get_connectors(conn_a,op,conn_b)
    conn_l = get_connector(conn_a) #ny
    conn_r = get_connector(conn_b) #ny

    name_c="#{conn_l.name}#{op}#{conn_r.name}"

    conn_c=Connector.new(name_c)
    @connectors[name_c]=conn_c
    [conn_l,conn_r,conn_c]
  end

  def get_connector(conn) #ny
    if conn.is_a? Connector
      return conn
    elsif conn.is_a? ArithmeticConstraint
      return conn.out
    end
  end
