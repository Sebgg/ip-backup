...
#
#   def initialize
#     @logicParser = Parser.new("bools") do
# 	  @variables = Hash.new()
#       token(/\s+/)         # ta bort whitespace
#       token(/\w+/) {|w|w}  # matchar ord (klarar inte av siffror och tecken)
#
#       # Börjar med att kolla om resultatet är ett uttryck eller tilldelning
#       start :valid do
#         match(:assign)
#         match(:expr)
#       end
#
#       # Matchar tilldelning ( med "set variabelnamn uttryck" )
#       rule :assign do
#         #sätter variabelnamn till ett värde i en hash
#         match('set', :var, :expr)   {|x, a, b | @variables[a.intern] = b}
#         match('satt' , :var, :expr) {|x, a, b | @variables[a.intern] = b}
#       end
#
#       # Matchar två uttryck med operatorer mellan
#       rule :expr do
#         match(:expr, 'or', :expr)   {|a, x, b | a or b }
#         match(:expr, 'and', :expr)  {|a, x, b | a and b }
#         match(:expr, 'eller', :expr){|a, x, b | a or b }
#         match(:expr, 'och', :expr)  {|a, x, b | a and b }
#         match('not',:expr)          {|x, a    | not a }
#         match('inte',:expr)         {|x, a    | not a }
#         match(:term) #(ett uttryck kan även vara en term)
#       end
#
#       # Matchar antingen true, false, eller en variabel
#       rule :term do
#         match('true')   {|a| true}
#         match('sant')   {|a| true}
#         match('false')  {|a| false}
#         match('falskt') {|a| false}
#         match(:var)
#       end
#
#       # Matchar alla ord i en hash och hämtar ut värdet
#       rule :var do
#         match(/[A-Za-z]+/) {|a| @variables[a.intern] != nil ? @variables[a.intern] : a}
#       end
#     end
#
...
#   end
#
#   # Rekursiv funktion som kör parse på de ord man skickar in
#   def parse
#     print "]]] evaluate >>> "
#     str=gets
#     if str == "exit\n" # stänger av programmet om man skriver exit
